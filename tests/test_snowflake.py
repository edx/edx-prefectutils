...
def test_load_s3_data_to_snowflake_defaults_success(mock_sf_connection):  # noqa: F811
    mock_cursor = mock_sf_connection.cursor()
    ...
    with Flow("test") as f:
        snowflake.load_s3_data_to_snowflake(
            ...
            sf_file_format="test file format",
            full_rebuild=False,
            uploaded_files=None
        )
    state = f.run()
...
def test_load_s3_data_to_snowflake_rebuild_success(mock_sf_connection):  # noqa: F811
    ...
    mock_cursor.execute.assert_has_calls(
        [
            ...
            mock.call("\n        CREATE OR REPLACE STAGE test_database.test_schema.test_table_stage\n            URL = 's3://edx-test/test/'\n            STORAGE_INTEGRATION = test_storage_integration\n            FILE_FORMAT = (test file format);\n        "),  # noqa
            mock.call("\n        CREATE OR REPLACE PIPE test_database.test_schema.test_table_pipe\n        AUTO_INGEST = FALSE\n        COMMENT = 'Automatically generated by prefect_utils.'\n        AS\n            COPY INTO test_database.test_schema.test_table (origin_file_name, origin_file_line, origin_str, properties)\n            FROM (\n            SELECT\n              metadata$filename,\n              metadata$file_row_number,\n              t.$1,\n              -- Insert JSON into the VARIANT column only if it can be parsed.\n              CASE\n                WHEN CHECK_JSON(t.$1) IS NULL\n                THEN t.$1\n                ELSE NULL\n              END\n            FROM @test_database.test_schema.test_table_stage t)\n          ON_ERROR=CONTINUE\n          FILE_FORMAT=(test file format);\n        "),  # noqa
            mock.call('\n        ALTER PIPE test_database.test_schema.test_table_pipe REFRESH\n        ')
        ]
    )


def test_load_s3_data_to_snowflake_retries(mock_sf_connection):  # noqa: F811
    mock_cursor = mock_sf_connection.cursor()
    mock_execute = mock.Mock(side_effect=ProgrammingError())
    ...
    # Snowflake errors should cause the whole task to retry 3 times before failing.
